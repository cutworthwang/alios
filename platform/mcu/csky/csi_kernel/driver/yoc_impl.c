...
#ifndef YUNOS_CONFIG_STD_MALLOC
...
#define DYNTICKLESS_LDCNT_PER_TICK  (SYSTEM_CLOCK/CONFIG_SYSTICK_HZ)
#define DYNTICKLESS_TICKS_DEFAULT   (10 * CONFIG_SYSTICK_HZ)
...
#ifdef CONFIG_LPM_TICKLESS_CNTTIM
        extern timer_handle_t count_timer;
        csi_timer_get_current_value(count_timer, &cur_cnt);
#else
        extern timer_handle_t system_timer;
        csi_timer_get_current_value(system_timer, &cur_cnt);
        csi_timer_get_load_value(system_timer, &max_cnt);
#endif
        elapsed_ticks = (~cur_cnt - ~g_prev_cnt) / DYNTICKLESS_LDCNT_PER_TICK;
#else
    if (yunos_bsp_intc_get_pending_irq(DYNTICKLESS_TIMER)) {
...
void soc_tick_interrupt_set(tick_t next_ticks, tick_t elapsed_ticks)
{
    ...
    if (next_ticks == (tick_t) - 1) {
        ...
        csi_timer_set_timeout(system_timer, ticks_from_last_interrupt * (1000000 / CONFIG_SYSTICK_HZ));
        csi_timer_start(system_timer, SYSTEM_CLOCK);
...
void soc_sys_mem_init(void)
{
    ...
    yunos_mm_region_init(&hobbit_mm_region_head,regions, 1);
#endif
}
...
void *soc_mm_alloc(size_t size)
{
    ...
    err = yunos_mm_bf_alloc(&hobbit_mm_region_head,&ptmp,size,alloctor);

    free_mem = yunos_mm_region_get_free_size(&hobbit_mm_region_head);
...
{
    yunos_mm_bf_free(&hobbit_mm_region_head,mem);
}
#endif//#ifndef YUNOS_CONFIG_STD_MALLOC
#endif//#if (YUNOS_CONFIG_KOBJ_DYN_ALLOC > 0)
...
{
    printf("stack overflow: %s!\n", g_active_task->task_name);
    while(1);
}

yunos_err_proc_t g_err_proc = yoc_err_proc;
