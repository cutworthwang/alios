...
extern int32_t get_usart_info(pin_t pin_tx, pin_t pin_rx, int32_t *spu_idx, int32_t *pgpio_tx, int32_t *pgpio_rx);
...
void ck_spu_usart_irqhandler(int32_t idx,spu_dev_signal_e signal)
{
    ...
    if((int_status & USART_INT_TX_DONE)!=0){
        priv->tx_busy = false;
        if(priv->cb_event){
          priv->cb_event(USART_EVENT_SEND_COMPLETE,priv->cb_arg);
        }
    }
    ...
    if((int_status & USART_INT_RX_DONE)!=0){
        ...
        priv->rx_busy = false;
        if(priv->cb_event){
          priv->cb_event(USART_EVENT_RECEIVE_COMPLETE, priv->cb_arg);
        }
    }
    ...
    if((int_status & USART_INT_PARITY_ERR)!=0){
        if(priv->cb_event){
          priv->cb_event(USART_EVENT_RX_PARITY_ERROR, priv->cb_arg);
        }
    }
    if((int_status & USART_INT_STOP_ERR)!=0){
        if(priv->cb_event){
          priv->cb_event(USART_EVENT_RX_FRAMING_ERROR,priv->cb_arg);
        }
    }
...
usart_handle_t csi_usart_initialize(pin_t tx, pin_t rx, usart_event_cb_t cb_event, void *cb_arg)
{
    ...
    usart_idx = get_usart_info(tx, rx, &spu_idx, &pgpio_tx, &pgpio_rx);
    if(usart_idx<0){
      return NULL;
    }
