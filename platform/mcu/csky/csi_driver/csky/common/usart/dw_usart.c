...
/**
  ...
  \param[in]   usart_priv usart private to operate.
*/
static void dw_usart_intr_recv_line(dw_usart_priv_t *usart_priv)
{
    dw_usart_reg_t *addr = (dw_usart_reg_t *)(usart_priv->base);
    ...
    if (lsr_stat & DW_LSR_BI) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(USART_EVENT_RX_BREAK, usart_priv->cb_arg);
        }
        return;
    }
    ...
    if (lsr_stat & DW_LSR_FE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(USART_EVENT_RX_FRAMING_ERROR, usart_priv->cb_arg);
        }
        return;
    ...
    if (lsr_stat & DW_LSR_PE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(USART_EVENT_RX_PARITY_ERROR, usart_priv->cb_arg);
        }
        return;
    ...
    if (lsr_stat & DW_LSR_OE) {
        if (usart_priv->cb_event) {
            usart_priv->cb_event(USART_EVENT_RX_OVERFLOW, usart_priv->cb_arg);
        }
        return;
...
/**
  ...
  \param[in]   usart_priv usart private to operate.
*/
static void dw_usart_intr_char_timeout(dw_usart_priv_t *usart_priv)
{
    if ((usart_priv->rx_total_num != 0) && (usart_priv->rx_buf != NULL)) {
...
void dw_usart_irqhandler(int32_t idx)
{
    ...
    switch (intr_state) {
        ...
        case DW_IIR_RECV_LINE:
            dw_usart_intr_recv_line(usart_priv);
            break;
        case DW_IIR_CHAR_TIMEOUT:
            dw_usart_intr_char_timeout(usart_priv);
            break;
        default:
            break;
    }
