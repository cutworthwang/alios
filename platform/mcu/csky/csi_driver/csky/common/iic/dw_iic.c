...
extern int32_t target_iic_init(pin_t scl, pin_t sda, uint32_t *base, uint32_t *irq);
...
*/
static void dw_iic_intr_tx_empty(dw_iic_priv_t *iic_priv, uint32_t intr_stat)
{
...
/**
  ...
  \param[in]   iic_priv pointer to iic private.
*/
static void dw_iic_intr_rx_full(dw_iic_priv_t *iic_priv, uint32_t intr_stat)
{
    dw_iic_reg_t *addr = (dw_iic_reg_t *)(iic_priv->base);
...
void dw_iic_irqhandler(int32_t idx)
{
    ...
    switch (iic_priv->status) {
        ...
        case IIC_STATE_DATASEND: {
            dw_iic_intr_tx_empty(iic_priv, intr_stat);
            break;
        }

        /* wait for data from slave */
        case IIC_STATE_WFDATA: {
            dw_iic_intr_rx_full(iic_priv, intr_stat);
            break;
        }
        ...
        default: {
            ...
            if (iic_priv->cb_event) {
                iic_priv->cb_event(I2C_EVENT_ARBITRATION_LOST, iic_priv->cb_arg);
            }
        }
    }
}
