...
extern int32_t target_spi_init(pin_t mosi, pin_t miso, pin_t sclk, pin_t ssel, uint32_t *base, uint32_t *irq);
...
static int32_t dw_spi_ss_init(dw_spi_priv_t *spi_priv)
{
    ...
    pgpio_pin_handle = csi_gpio_pin_initialize(spi_priv->ssel, NULL, NULL);
...
int32_t csi_spi_config_datawidth(spi_handle_t handle, int32_t datawidth)
{
    ...
    return ERR_SPI(EDRV_SPI_DATA_BITS);
}
...
int32_t csi_spi_config_format(spi_handle_t handle, spi_format_e format)
{
    ...
    switch (format) {
        ...
        default:
            return ERR_SPI(EDRV_SPI_FRAME_FORMAT);
    }
...
int32_t csi_spi_config_bit_order(spi_handle_t handle, spi_bit_order_e order)
{
    ...
    return ERR_SPI(EDRV_SPI_BIT_ORDER);;
}
...
int32_t csi_spi_config_ss_mode(spi_handle_t handle, spi_ss_mode_e ss_mode)
{
    ...
    switch (ss_mode) {
        ...
        default:
            return ERR_SPI(EDRV_SPI_SS_MODE);
    }
                       ...
                       int32_t          bit_width)
{
    ...
    ret = csi_spi_config_baudrate(handle, baud, sysclk);
    if (ret < 0) {
        return ret;
    }
