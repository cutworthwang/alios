...
typedef void (*norflash_event_cb_t)(norflash_event_e event);   ///< Pointer to \ref norflash_event_cb_t : NORFLASH Event call back.
...
/**
  \brief       get norflash handle count.
  \return      norflash handle count
*/
int32_t csi_norflash_get_instance_count(void);
...
*/
norflash_capabilities_t csi_norflash_get_capabilities(norflash_handle_t handle);
...
/**
  ...
  \param[in]   idx  must not exceed return value of csi_norflash_get_instance_count()
