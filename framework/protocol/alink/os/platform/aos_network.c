...
static int network_create_socket( pplatform_netaddr_t netaddr, int type,
                                  struct sockaddr_in *paddr, long *psock)
{
    struct hostent *hp;
    struct in_addr in;
    uint32_t ip;
    ...
    *psock = socket(AF_INET, type, 0);
    ...
    if (0 != setsockopt(*psock, SOL_SOCKET, SO_REUSEADDR, &opt_val,
                        sizeof(opt_val))) {
        perror("setsockopt");
        close((int)*psock);
        return -1;
    ...
    if (type == SOCK_DGRAM) {
        if (0 != setsockopt(*psock, SOL_SOCKET, SO_BROADCAST, &opt_val,
                            sizeof(opt_val))) {
            perror("setsockopt");
            close((int)*psock);
            return -1;
...
void *platform_udp_server_create(_IN_ uint16_t port)
{
    ...
    if (-1 == bind(server_socket, (struct sockaddr *)&addr,
...
void *platform_udp_multicast_server_create(pplatform_netaddr_t netaddr)
{
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (char *)&option,
                   sizeof(option)) < 0) {
        perror("setsockopt");
...
int platform_tcp_send(_IN_ void *handle, _IN_ const char *buffer,
                      _IN_ uint32_t length)
{
    int bytes_sent;
...
int platform_tcp_recv(_IN_ void *handle, _OUT_ char *buffer,
                      _IN_ uint32_t length)
{
    int bytes_received;
