...
#endif
static int lws_gettimeofday(struct timeval *now, void *param)
{
    uint64_t time_ms = aos_now_ms();
    now->tv_sec = (long)(time_ms / 1000);
    now->tv_usec = (long)(time_ms % 1000) * 1000;
    return 0;
}
...
int ssl_pm_new(SSL *ssl)
{
   ...
   // mbedtls_entropy_init(&ssl_pm->entropy);
    ...
    // ret = mbedtls_ctr_drbg_seed(&ssl_pm->ctr_drbg, NULL, &ssl_pm->entropy, pers, pers_len);
    // if (ret) {
    ...
    mbedtls_ssl_conf_rng(&ssl_pm->conf, ssl_random/*mbedtls_ctr_drbg_random*/, NULL/*&ssl_pm->ctr_drbg*/);
...
int ssl_pm_handshake(SSL *ssl)
{
    ...
    SSL_DEBUG(SSL_PLATFORM_ERROR_LEVEL, "ssl_pm_reload_crt() return -0x%x", -ret);
    if (ret)
        return 0;
